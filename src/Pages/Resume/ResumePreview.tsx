...
export default function ITResumeBuilder() {
  ...
  const calculateATSScore = (data: ResumeData) => {
    let score = 0;
    if (data.name && data.name !== 'Your Name') score += 10;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`${currentResumeData.name}_Resume.pdf`);
      
      alert('✅ Resume downloaded successfully! Ready to apply for jobs!');
    } catch (error) {
  ...
  // Convert user profile to resume data
  useEffect(() => {
    if (userprofile) {
      const mappedData: ResumeData = {
        name: userprofile.name || 'Your Name',
        title: userprofile.experience?.[0]?.title || 'Software Developer',
        email: userprofile.email || 'email@example.com',
        phone: userprofile.phone || '+91 00000 00000',
        location: userprofile.location || 'Location',
        github: userprofile.links?.find((l: any) => l.platform === 'GitHub')?.url || '',
        linkedin: userprofile.links?.find((l: any) => l.platform === 'LinkedIn')?.url || '',
        portfolio: userprofile.portfolio || '',
        summary: userprofile.bio || userprofile.resume_objective || 'Professional summary',
        experience: userprofile.experience?.map((exp: any) => ({
          title: exp.title || '',
          company: exp.company || '',
          location: userprofile.location || '',
          period: exp.year || '',
          achievements: exp.desc ? [exp.desc] : []
        })) || [],
        education: userprofile.education?.[0] ? {
          degree: userprofile.education[0].degree || 'B.Tech in Computer Science',
          institution: userprofile.education[0].school || userprofile.institute || 'University',
          location: userprofile.location || '',
          period: userprofile.education[0].year || ''
        } : {
          degree: 'B.Tech in Computer Science',
          institution: userprofile.institute || 'University',
          location: userprofile.location || '',
          period: `${userprofile.yearOfStudy || 1}st Year`
        },
        skills: userprofile.skills || [],
        certifications: userprofile.achievements || []
      };
      setResumeData(mappedData);
    } else {
      // Use default data if no profile
      setResumeData(resumeDataByField[selectedField]);
    }
  }, [userprofile, selectedField]);
  ...
  const currentResumeData = resumeData || resumeDataByField[selectedField];

  const ResumeTemplate1 = ({ color, gradient }: TemplateProps) => (
    <div ref={resumeRef} className="bg-white w-full aspect-[8.5/11] shadow-lg">
      <div className="flex h-full">
        <div className={`w-2/5 bg-gradient-to-br ${gradient} text-white p-6`}>
          <div className="mb-6">
            ...
            <h1 className="text-2xl font-bold text-center mb-1">{currentResumeData.name}</h1>
            <p className="text-center text-sm opacity-90">{currentResumeData.title}</p>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex items-start gap-1.5">
                ...
                <span className="break-all">{currentResumeData.email}</span>
              </div>
              <div className="flex items-start gap-1.5">
              ...
              {currentResumeData.github && (
                <div className="flex items-start gap-1.5">
                  <Github className="w-3 h-3 mt-0.5 flex-shrink-0" />
                  <span className="break-all">{currentResumeData.github}</span>
                </div>
              )}
              {currentResumeData.linkedin && (
                <div className="flex items-start gap-1.5">
                  <Linkedin className="w-3 h-3 mt-0.5 flex-shrink-0" />
                  <span className="break-all">{currentResumeData.linkedin}</span>
                </div>
              )}
              {currentResumeData.portfolio && (
                <div className="flex items-start gap-1.5">
                  <Globe className="w-3 h-3 mt-0.5 flex-shrink-0" />
                  <span className="break-all">{currentResumeData.portfolio}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="text-xs space-y-1">
              <p className="font-semibold">{currentResumeData.education.period}</p>
              <p className="font-bold">{currentResumeData.education.degree}</p>
              <p className="opacity-90">{currentResumeData.education.institution}</p>
              <p className="opacity-80">{currentResumeData.education.location}</p>
            </div>
          </div>
          ...
          {currentResumeData.certifications && currentResumeData.certifications.length > 0 && (
            <div>
              ...
              <ul className="space-y-1 text-xs">
                {currentResumeData.certifications.map((cert, idx) => (
        ...
        <div className="flex-1 p-6 bg-white">
          <div className="mb-6">
            ...
            <p className="text-xs text-gray-700 leading-relaxed">{currentResumeData.summary}</p>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-4">
              {currentResumeData.experience.map((exp, idx) => (
                <div key={idx}>
  ...
  const ResumeTemplate2 = ({ color, gradient }: TemplateProps) => (
    <div ref={resumeRef} className="bg-white w-full aspect-[8.5/11] shadow-lg p-8">
      <div className="text-center mb-6 pb-4" style={{ borderBottom: `3px solid ${color}` }}>
        ...
        <h1 className="text-3xl font-bold text-gray-800 mb-1">{currentResumeData.name}</h1>
        <p className="text-lg font-semibold mb-3" style={{ color }}>{currentResumeData.title}</p>
        <div className="flex justify-center gap-3 text-xs text-gray-600 flex-wrap">
          <span className="flex items-center gap-1"><Mail className="w-3 h-3" />{currentResumeData.email}</span>
          <span className="flex items-center gap-1"><Phone className="w-3 h-3" />{currentResumeData.phone}</span>
          <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{currentResumeData.location}</span>
          {currentResumeData.github && <span className="flex items-center gap-1"><Github className="w-3 h-3" />{currentResumeData.github}</span>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div>
          ...
          <div className="text-xs">
            <p className="font-bold text-gray-800">{currentResumeData.education.degree}</p>
            <p className="font-semibold" style={{ color }}>{currentResumeData.education.institution}</p>
            <p className="text-gray-600">{currentResumeData.education.location} • {currentResumeData.education.period}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {currentResumeData.skills.slice(0, 8).map((skill, idx) => (
      ...
      {currentResumeData.certifications && currentResumeData.certifications.length > 0 && (
        <div className="mt-4">
          ...
          <div className="text-xs text-gray-700">
            {currentResumeData.certifications.map((cert, idx) => (
              <span key={idx}>{cert}{idx < currentResumeData.certifications!.length - 1 ? ' • ' : ''}</span>
            ))}
          </div>
        </div>
      )}
  ...
  const ResumeTemplate3 = ({ color, gradient }: TemplateProps) => (
    <div ref={resumeRef} className="bg-gray-50 w-full aspect-[8.5/11] shadow-lg">
      <div className={`bg-gradient-to-r ${gradient} text-white p-6`}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold mb-1">{currentResumeData.name}</h1>
            <p className="text-lg opacity-90 mb-2">{currentResumeData.title}</p>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="space-y-4">
            <div className="bg-white p-3 rounded-lg shadow-sm">
              ...
              <div className="flex flex-wrap gap-1.5">
                {currentResumeData.skills.slice(0, 10).map((skill, idx) => (
            ...
            <div className="bg-white p-3 rounded-lg shadow-sm">
              ...
              <div className="text-xs">
                <p className="font-bold text-gray-800">{currentResumeData.education.degree}</p>
                <p className="font-semibold" style={{ color }}>{currentResumeData.education.institution}</p>
                <p className="text-gray-600">{currentResumeData.education.period}</p>
              </div>
            </div>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">Choose Your template for Your Profession</p>
            </div>
            <div className="flex flex-col items-end gap-2">
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-600">ATS Score:</span>
                <div className={`px-4 py-2 rounded-full font-bold text-lg ${
                  calculateATSScore(currentResumeData) >= 80 ? 'bg-green-100 text-green-700' :
                  calculateATSScore(currentResumeData) >= 60 ? 'bg-yellow-100 text-yellow-700' :
                  'bg-red-100 text-red-700'
                }`}>
                  {calculateATSScore(currentResumeData)}%
                </div>
              </div>
              <p className="text-xs text-gray-500">
                {calculateATSScore(currentResumeData) >= 80 ? '🎉 Excellent! ATS-friendly' :
                 calculateATSScore(currentResumeData) >= 60 ? '⚠️ Good, but can improve' :
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {[1, 2, 3].map((templateId) => (
                <div key={templateId} className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-2xl transition-all transform hover:-translate-y-1 relative">
                  ...
                  <div className="p-4">
                    <h3 className="font-bold text-gray-900 mb-1">{resumeData?.name || 'Your Name'}</h3>
                    <p className="text-xs text-gray-600 mb-3">{resumeData?.title || 'Your Title'}</p>
                    ...
                    <div className="flex gap-2">
                      <button 
                        ...
                        className="flex-1 bg-gradient-to-r from-cyan-500 to-teal-500 text-white text-xs font-semibold py-2 px-2 rounded-lg hover:shadow-lg transition-all flex items-center justify-center gap-1"
                      >
                        <Eye className="w-3 h-3" /> Preview
                      </button>
                      <button 
                        onClick={() => { setSelectedTemplate(templateId); handleDownloadPDF(); }}
                        disabled={downloading}
                        className="flex-1 bg-gradient-to-r from-green-500 to-emerald-500 text-white text-xs font-semibold py-2 px-2 rounded-lg hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-1"
